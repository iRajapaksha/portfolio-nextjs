...
export const TimelineItem =({date,title,subtitle, link, tag, isCourse}:TimelineItemProps)=>{
    return(
            ...
            <div className="flex gap-x-2 " style={{transform:`${isCourse ? "translateX(-45px)" : ""}`}}>
                <div className="flex flex-col gap-0.5">
                    ...
                    <div className="flex items-center gap-2 w-min">
                        {
                            link ? <Link href={link}><TimelineItemBody link={link} tag={tag} subtitle={subtitle}/></Link> : subtitle
                        }
                    </div>
                </div>
            </div>
